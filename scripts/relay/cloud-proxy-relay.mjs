...
const RAW_AGENT_SERVER_URL = process.env.AGENT_SERVER_URL ?? "http://127.0.0.1:18000";
const AGENT_SERVER_URL =
  RAW_AGENT_SERVER_URL === "" || /^(none|off|disabled)$/i.test(RAW_AGENT_SERVER_URL)
    ? null
...
const AGENT_PREFIXES = ["/api", "/sockets", "/server_info", "/health", "/alive", "/openapi.json", "/docs", "/redoc", "/ready"];

function sendJson(res, status, obj, extraHeaders = {}) {
  const body = JSON.stringify(obj);
  const headers = { "content-type": "application/json", "access-control-allow-origin": ALLOW_ORIGIN, ...extraHeaders };
...
// (the user message lands on a later page that never loads). We do two
...
// delayed) response rather than a silent empty render.
...
async function fetchUpstream(options) {
  ...
  return new Promise((resolve, reject) => {
    const upstream = lib(
      { hostname: url.hostname, port: url.port || (isTls ? 443 : 80), path: url.pathname + url.search, method, headers: reqHeaders },
      (up) => {
        const chunks = [];
    ...
    upstream.setTimeout(timeoutMs, () => upstream.destroy(new Error(`upstream timeout after ${timeoutMs}ms`)));
...
// the retry helper can re-issue the request on 429 without the client
// having consumed the first (rate-limited) response. Concurrency is capped
// so the open-burst is paced under the cloud rate limit. Backoff is patient
// (up to ~8s) and honors Retry-After, because the cloud's 429 window can
// outlast a short retry sequence — giving up too early surfaces the 429 to
// the client, where even a retried query can degrade a UI section to empty.
  ...
  try {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
      ...
      if (attempt === maxRetries) {
        console.warn(`[cloud-proxy] 429 persisted after ${maxRetries} retries: ${options.url.pathname}${options.url.search}`);
        return result;
      }
      ...
      if (retryAfter && /^\d+$/.test(retryAfter)) {
        ...
        delay = Math.min(parseInt(retryAfter, 10) * 1000, 10_000);
      } else {
        // Exponential backoff: 0.5s, 1s, 2s, 4s, 8s — long enough to clear
        // the observed per-key rate-limit window without making the
        // conversation-open burst feel frozen.
        delay = Math.min(500 * 2 ** attempt, 8_000);
      }
      await sleep(delay);
    }
    return last;
...
function applyCors(res) {
  ...
  res.setHeader("access-control-allow-headers", "Content-Type, X-Session-API-Key, X-Org-Id, Authorization, X-Expose-Secrets");
...
async function forwardToCloud(envelope, res) {
  ...
  if (["POST", "PATCH", "PUT"].includes(method) && envelope.body !== undefined && envelope.body !== null) {
    reqHeaders["content-type"] = reqHeaders["content-type"] || "application/json";
  }

  const bodyBytes = (envelope.body !== undefined && envelope.body !== null)
    ? Buffer.from(typeof envelope.body === "string" ? envelope.body : JSON.stringify(envelope.body))
    : null;
  if (bodyBytes) reqHeaders["content-length"] = String(bodyBytes.length);

  const timeoutMs = (envelope.timeout_seconds ? envelope.timeout_seconds * 1000 : 30_000);

  try {
    const result = await fetchWithRetry({ url, method, reqHeaders, bodyBytes, timeoutMs });
    res.writeHead(result.statusCode, { ...result.headers, "access-control-allow-origin": ALLOW_ORIGIN });
    res.end(result.body);
  } catch (err) {
    console.error("[cloud-proxy] upstream error:", err.message);
    if (!res.headersSent) sendJson(res, 502, { error: "upstream_error", detail: err.message });
    else res.destroy();
  }
}
...
function proxyToAgentServer(req, res) {
  ...
  const lib = isTls ? httpsRequest : httpRequest;
  const upstream = lib(
    { hostname: url.hostname, port: url.port || (isTls ? 443 : 80), path: url.pathname + url.search, method: req.method, headers: { ...req.headers, host: url.host } },
    (up) => {
  ...
  upstream.on("error", (err) => {
    ...
    if (!res.headersSent) sendJson(res, 502, { error: "agent_server_unreachable", detail: err.message });
    else res.destroy();
  });
  req.pipe(upstream);
}
...
function proxyToCloud(req, res) {
  ...
  req.on("end", async () => {
    ...
    if (bodyBytes.length) reqHeaders["content-length"] = String(bodyBytes.length);
    try {
      ...
      res.writeHead(result.statusCode, { ...result.headers, "access-control-allow-origin": ALLOW_ORIGIN });
      res.end(result.body);
      console.log(`[proxyToCloud] ${req.method} ${url.pathname}${url.search} -> ${result.statusCode} ct=${result.headers["content-type"] || "?"}`);
    } catch (err) {
      console.error("[cloud-agent-proxy] upstream error:", err.message);
      if (!res.headersSent) sendJson(res, 502, { error: "cloud_unreachable", detail: err.message });
      else res.destroy();
    }
  });
  req.on("error", (err) => {
...
function proxyWebSocketToCloud(req, socket, head) {
  ...
  const upstream = lib(
    { hostname: url.hostname, port: url.port || (isTls ? 443 : 80), path: url.pathname + url.search, method: req.method, headers: reqHeaders },
  );
  upstream.on("upgrade", (up, upSocket, upHead) => {
    console.log(`[cloud-ws] upgrade received from cloud, piping bidirectionally`);
    socket.write(
    ...
    upSocket.on("error", (e) => console.error("[cloud-ws] upSocket error:", e.message));
    socket.on("error", (e) => console.error("[cloud-ws] clientSocket error:", e.message));
    socket.on("close", () => { console.log("[cloud-ws] client socket closed"); upSocket.destroy(); });
    upSocket.on("close", () => { console.log("[cloud-ws] upstream socket closed"); if (!socket.destroyed) socket.destroy(); });
  });
  upstream.on("response", (up) => {
    ...
    up.on("data", (c) => (body += c));
    up.on("end", () => {
      console.error(`[cloud-ws] cloud returned HTTP ${up.statusCode} (not upgrade): ${body.slice(0, 200)}`);
      if (!socket.destroyed) {
        socket.write(`HTTP/1.1 ${up.statusCode} ${up.statusMessage || ""}\r\n`);
        [...Object.entries(up.headers)].forEach(([k, v]) => socket.write(`${k}: ${v}\r\n`));
        socket.write("\r\n");
        socket.write(body);
        socket.end();
      }
...
async function serveStatic(req, res) {
  if (!STATIC_DIR) { sendJson(res, 404, { error: "not_found" }); return; }
  let rel = decodeURIComponent(new URL(req.url, "http://x").pathname);
  if (rel.includes("..")) { sendJson(res, 403, { error: "forbidden" }); return; }
...
const server = createServer((req, res) => {
  ...
  if (req.method === "POST" && req.url.split("?")[0] === CLOUD_PROXY_PATH) {
    if (PROXY_KEY) {
      ...
      if (key !== PROXY_KEY) { sendJson(res, 401, { error: "invalid_proxy_key" }); return; }
    }
    let buf = "";
    req.on("data", (c) => (buf += c));
    req.on("end", () => {
      let envelope;
      try { envelope = JSON.parse(buf || "{}"); }
...
server.on("upgrade", (req, socket, head) => {
  ...
  const pathOnly = req.url.split("?")[0];
  const isSocketPrefix = pathOnly === "/sockets" || pathOnly.startsWith("/sockets/");
  if (!isSocketPrefix) {

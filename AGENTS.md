...
- Touch swipe gestures to open/close panels (no icon tap required): a reusable `useSwipeGesture` hook (`src/hooks/use-swipe-gesture.ts`) attaches native `touchstart`/`touchmove`/`touchend` listeners and fires `onSwipe("left"|"right")` only when the gesture is (a) dominantly horizontal (vertical scrolls are never hijacked), (b) travels past a 60px threshold, and (c) — for *opening* gestures — begins within a 28px screen-edge zone (`startEdge: "left"|"right"`). `preventDefault` is called only after a swipe commits, so taps and in-progress scrolls are not disrupted. Close gestures are scoped to a `targetRef` element so a swipe on the chat body never closes an open panel. Pointer/mouse is intentionally not handled — desktop keeps the toggle buttons and resize handle. Wiring: `ConversationMain` — left-edge swipe-right opens the mobile sidebar drawer (`useSidebarMobileNav().open()`), right-edge swipe-left opens the right panel (desktop side-drawer via store setters; mobile via `navigate("/conversations/:id/panel")`), and swipe-right on the desktop panel body closes it. `sidebar.tsx` — swipe-left on the mobile drawer closes it (`closeMobileNav()`). `conversation-mobile-panel-page.tsx` — swipe-right navigates back to the conversation. The open gestures are enabled only when the target panel is closed (and vice-versa), and the sidebar open gesture is gated on `isSidebarRailHidden` (mobile-only). Test: `__tests__/hooks/use-swipe-gesture.test.tsx` (10 tests — edge-zone, threshold, direction filter, vertical-scroll non-interference, target scoping, listener cleanup, single-fire).
